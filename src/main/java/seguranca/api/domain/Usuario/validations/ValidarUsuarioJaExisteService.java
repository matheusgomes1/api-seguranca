...
public class ValidarUsuarioJaExisteService implements IValidacaoUsuario {
    @Autowired
    ...
    public void Validar(CadastroUsuarioDto dto) {
        var usuario = repository.findBylogin(dto.login());

        if (usuario != null)
            notificador.addNotificacao("Erro de domínio","Usuário já cadastrado.");
    }
}
